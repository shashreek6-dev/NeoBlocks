...
export default function GameBoard({
  ...
  return (
    <div
      id="game-board-grid"
      className="bg-[#0c0d14] p-1 border-[4px] border-[#ea00d9] shadow-[0_0_20px_rgba(234,0,217,0.7),inset_0_0_15px_rgba(0,0,0,0.8)] relative w-full aspect-square max-w-[365px] grid grid-cols-8 gap-1 select-none overflow-hidden rounded-2xl mx-auto z-10"
    >
      {/* Laser horizontal/vertical lines scanline motif */}
      <div className="absolute inset-0 bg-[linear-gradient(rgba(0,240,255,0.04)_1px,transparent_1px),linear-gradient(90deg,rgba(234,0,217,0.04)_1px,transparent_1px)] bg-[size:12px_12px] pointer-events-none z-0" />
      ...
      {board.map((row, rIdx) =>
        row.map((cell, cIdx) => {
          ...
          if (isFilled) {
            ...
            customStyle.color = getHexColor(cell);
